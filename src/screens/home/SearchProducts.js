  ...
  return (
    <>
        ...
        <div className="my-container my-10">
          {isFetching ? <ProductSkeleton /> : data.count > 0 ? (
          <>
          ...
          <Pagination page={parseInt(page)} perPage={data.perPage} count={data.count} path={`search-products/${keyword}`} theme="light" />
          </>) : <p className="alert-danger flex items-center justify-center text-lg">No products found for #{keyword} keyword.</p>}
        </div>
    </>
  )
}
